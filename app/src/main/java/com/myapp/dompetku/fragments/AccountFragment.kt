...
class AccountFragment : Fragment() {
    ...
    private var dbRef: DatabaseReference = FirebaseDatabase.getInstance()
        .getReference("users")
        .child(uid!!)
        .child("transactions")
    ...
    private fun accountDetails() {
        ...
        if (user != null) {
            ...
            val name = displayName ?: email?.substringBefore("@") ?: "User"

            tvName.text = name
            tvEmail.text = email
            tvPicture.text = name.firstOrNull()?.uppercase() ?: "U"
            ...
            if (user.isEmailVerified) {
                ...
                verified.setOnClickListener {
                    Toast.makeText(requireContext(), "Your account is verified!", Toast.LENGTH_SHORT).show()
                }
            } else {
